program ising
  ...
  implicit none
  integer, parameter :: nsnapshots = 250, L = 20
  ...
  do temp_index = 1, end_count
    ...
    do k = 1, nsnapshots
      ...
      write(filename, '(a,i3.3,a,i3.3,a)') 'dat_', int(temp*100)+1, '_', snapshot_counter, '.dat'
      open(unit=10, file=filename, status='replace')
      do j = 1, L
        write(10, '(20i3)') spin_out(j,:)
      end do
      close(10)
    end do
...
contains
  subroutine ising_sim(T, spin_matrix_in, spin_matrix_out) ! Generate snapshots for a chosen T
    ...
    integer, parameter :: L = 20 ! Lattice size
    ...
    real(kind=16), parameter :: coup = 0.5d0 ! Coupling constant
